...
def run_evaluation_main(test_edges, prediction_data, true_data, threshold, header):
    ...
    y_pred = np.zeros(len(prediction_data), dtype=np.int32)  # 模型预测的标签值（高于threshold的分数值记为1）

    for i, _ in enumerate(prediction_data):
        if prediction_data[i] >= threshold:  # >=
            y_pred[i] = 1

    y_true = np.array(true_data)  # 正确的label值
    y_scores = np.array(prediction_data)  # 模型预测的分数值
