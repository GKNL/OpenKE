...
class LinkPrediction(torch.nn.Module):
    ...
    def __init__(self, opt, pretrained_node_embeddings):
        """
        Args:
    ...
    def __predict_via_dot_product(self, edge):
        ...
        score = torch.bmm(source_vec, target_vec)  # 张量乘法
        score = torch.sigmoid(score).data.cpu().numpy().tolist()[0][0][0]

        return score

    def forward(self, triple):  # TODO:LinkPrediction实际上表示为网络中的一个层，后续需要根据loss来训练其weights参数？
...
def link_prediction_eval(
    ...
    #     "dot_product", pretrained_node_embeddings
    # )
    ...
    for data_type in ["valid", "test"]:
        ...
        for edge in edges:
            ...
            relation_id = int(edge[0])
            source_id = int(edge[1])
            target_id = int(edge[2])
            score = model.predict({
                'batch_h': torch.tensor([source_id]),
                'batch_t': torch.tensor([target_id]),
                'batch_r': torch.tensor([relation_id]),
                'mode': 'normal'
            })
            # score = -score
